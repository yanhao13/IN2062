...
class SquareLoss(FunctionNode):
    ...
    def _backward(gradient, *inputs):
        ...
        dx = (inputs[0] - inputs[1]) / inputs[0].size
        dw = (inputs[1] - inputs[0]) / inputs[0].size
        # multiply the upstream gradient with the local gradients
        return gradient * dx, gradient * dw
...
class SoftmaxLoss(FunctionNode):
    ...
    def _backward(gradient, *inputs):
        ...
        "*** YOUR CODE HERE (Q5) ***"
        # Obtain the derivatives and normalize
        # dLoss/dLogits
        dl_dx = (np.exp(log_probs) - inputs[1]) / inputs[0].shape[0]
